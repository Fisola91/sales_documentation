...
export default class extends Controller {
  ...
  connect(event) {
    ...
    let netTotals = document.querySelectorAll(".total")
    let netQuantities = document.querySelectorAll(".quantity")

    let totalCell = 0;
    let quantityCell = 0

    for (let i = 0; i < netTotals.length; i++) {
      const total = parseFloat(netTotals[i].innerText);
      const quantity = parseFloat(netQuantities[i].innerText);
      totalCell  +=  total
      quantityCell  +=  quantity
    }

    const totalPrice = document.querySelector(".grand-total")
    const totalQuantity = document.querySelector(".grand-total-quantity")

    totalPrice.classList.add("font-bold")
    totalQuantity.classList.add("font-bold")

    parseFloat(totalPrice.innerText = totalCell.toFixed(2));
    parseFloat(totalQuantity.innerText = quantityCell);
  ...
  createSummaryRow() {
    ...
    console.log(form)
    const name = form.elements.order_name.value;
    const quantity =form.elements.order_quantity.value;
    const unitPrice = form.elements.order_unit_price.value;
    const total = form.elements.order_total.value;
